...
# Testing Flags
results_dir="./results/"                #32 - Saves results here

aspect_ratio=1.0                        #33 - Aspect ratio of result images

eval=                                   #34 - If true, use eval mode during test time. Else, leave variable blank

num_test=50                             #35 - How many test images to run

sbatch <<EOT
#!/bin/bash
#SBATCH --job-name="P2P Test"           #46 - The name of the job as shown on Delta queue
...
srun python test.py --dataroot ${dataroot} --name ${name} --gpu_ids ${gpu_ids} --checkpoints_dir ${checkpoints_dir} --model ${model} --input_nc ${input_nc} --output_nc ${output_nc} --ngf ${ngf} --ndf ${ndf} --netD ${netD} --netG ${netG} --n_layers_D ${n_layers_D} --norm ${norm} --init_type ${init_type} --init_gain ${init_gain} ${no_dropout:+--no_dropout} --dataset_mode ${dataset_mode} ${serial_batches:+--serial_batches} --direction ${direction} --num_threads ${num_threads} --batch_size ${batch_size} --load_size ${load_size} --crop_size ${crop_size} --preprocess ${preprocess} ${no_flip:+--no_flip} --display_winsize ${display_winsize} --epoch ${epoch} --load_iter ${load_iter} ${verbose:+--verbose} --results_dir ${results_dir} --aspect_ratio ${aspect_ratio} ${eval:+--eval} --num_test ${num_test} >> p2p.out
